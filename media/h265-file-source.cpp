...
int H265FileSource::Play()
{
	...
	uint64_t clock = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
	if (0 == m_rtp_clock)
		m_rtp_clock = clock;
...
int H265FileSource::GetSDPMedia(std::string& sdp) const
{
    ...
    for(it = sps.begin(); it != sps.end(); ++it)
    {
        ...
        bytes = base64_encode((uint8_t*)base64, it->first, bytes);
		//base64[bytes] = '\0';
